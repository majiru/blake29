...
int blake2xs_init( blake2xs_state *S, const u64int outlen ) {
  return blake2xs_init_key(S, outlen, nil, 0);
}
...
int blake2xs_final(blake2xs_state *S, void *out, u64int outlen) {
  ...
  blake2s_state C[1];
  blake2s_param P[1];
  u16int xof_length = load16(&S->P->xof_length);
  u8int root[BLAKE2S_BLOCKBYTES];
  u64int i;
  ...
  for (i = 0; outlen > 0; ++i) {
    const u64int block_size = (outlen < BLAKE2S_OUTBYTES) ? outlen : BLAKE2S_OUTBYTES;
    /* Initialize state */
    ...
    if (blake2s_final(C, (u8int *)out + i * BLAKE2S_OUTBYTES, block_size) < 0) {
        return -1;
    }
    outlen -= block_size;
  }
